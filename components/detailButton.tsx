...
export default function ButtonDetail({ itemmu, onView }: ButtonDetailProps) {
  ...
  const generateReceiptContent = () => {
    ...
    receipt += `Catatan:${' '.repeat(4 - (itemmu.notes?.length ?? 0))}${itemmu.notes ?? '-'}\n`;
    receipt += '--------------------------------\n';
        ...
        <div className="flex flex-row w-full gap-2">
          <div className="flex flex-col w-1/2 ">
            ...
            <div>
              ...
              <p className="border p-2 rounded">{itemmu.notes}</p>
            </div>
            <div>
  ...
  const Content = () => (
    <div className="grid gap-4 py-4">
      ...
      <div style={{ display: 'none' }}>
        <div ref={thermalPrintRef} className="thermal-receipt" style={{ width: '80mm', fontFamily: 'monospace' }}>
          {status === "1" ? (
            <>
              <div style={{ textAlign: 'center', marginBottom: '10px' }}>
                <h3 style={{ margin: '5px 0', fontSize: '18px' }}>NUNIS WARUNG & KOFFIE</h3>
              </div>

              <div style={{ borderTop: '1px dashed #000', margin: '5px 0' }}></div>

              <div style={{ marginBottom: '10px' }}>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Pelanggan: {itemmu.user.nama}</p>
              </div>
              ...
              <div style={{ marginBottom: '10px' }}>
                {itemmu.detail_penjualan.map((item, index) => (
                  <div key={index} style={{ margin: '3px 0', fontSize: '16px' }}>
                    <p style={{ margin: '0' }}>{item.name} x {item.jumlah}</p>
                  </div>
                ))}
              </div>
              ...
              <div style={{ margin: '5px 0', fontSize: '16px' }}>
                <p style={{ margin: '0' }}>Catatan: {itemmu.notes || '-'}</p>
              </div>
            ...
            <>
              <div style={{ textAlign: 'center', marginBottom: '10px' }}>
                <h3 style={{ margin: '5px 0', fontSize: '18px' }}>NUNIS WARUNG & KOFFIE</h3>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Jl. Raya Trenggalek - Ponorogo No.Km.7</p>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Kec. Tugu, Kab. Trenggalek, Jawa Timur</p>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Telp: 085217645464</p>
              </div>

              <div style={{ borderTop: '1px dashed #000', margin: '5px 0' }}></div>

              <div style={{ marginBottom: '10px' }}>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>No. Faktur: {itemmu.faktur}</p>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Tanggal: {new Date(itemmu.tanggal).toLocaleDateString()}</p>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Kasir: Admin</p>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Pelanggan: {itemmu.user.nama}</p>
              </div>
                ...
                {itemmu.detail_penjualan.map((item, index) => (
                  <div key={index} style={{ margin: '3px 0', fontSize: '16px' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <p style={{ margin: '0' }}>{item.jumlah} x Rp. {item.subtotal / item.jumlah}</p>
                      <p style={{ margin: '0' }}>{formatCurrency(item.subtotal)}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ marginBottom: '5px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', margin: '2px 0', fontSize: '16px' }}>
                  <p style={{ margin: '0' }}>Subtotal:</p>
                  <p style={{ margin: '0' }}>{formatCurrency(itemmu.sub_total)}</p>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', margin: '2px 0', fontSize: '16px' }}>
                  <p style={{ margin: '0' }}>Diskon:</p>
                  <p style={{ margin: '0' }}>{formatCurrency(itemmu.diskon_rupiah)}</p>
                </div>
                <div style={{ borderTop: '1px dashed #000', margin: '5px 0' }}></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', margin: '5px 0', fontWeight: 'bold', fontSize: '18px' }}>
                  <p style={{ margin: '0' }}>TOTAL:</p>
                  <p style={{ margin: '0' }}>{formatCurrency(itemmu.total)}</p>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', margin: '2px 0', fontSize: '16px' }}>
                  <p style={{ margin: '0' }}>Bayar ({paymentMethod}):</p>
                  <p style={{ margin: '0' }}>{formatCurrency(cashAmount)}</p>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', margin: '2px 0', fontSize: '16px' }}>
                  <p style={{ margin: '0' }}>Kembalian:</p>
                  <p style={{ margin: '0' }}>{formatCurrency(change)}</p>
                </div>
              </div>
              ...
              <div style={{ textAlign: 'center', marginTop: '10px' }}>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>Terima kasih atas kunjungan Anda</p>
                <p style={{ margin: '2px 0', fontSize: '16px' }}>www.nunis.id</p>
              </div>
            </>
          )}
        </div>
