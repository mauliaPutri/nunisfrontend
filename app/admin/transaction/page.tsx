...
export default function TransactionPage() {
  ...
  const checkNewTransactions = async () => {
    try {
      ...
      // Periksa apakah ada transaksi baru
      if (sortedData.length > products.length) {
        const newTransactions = sortedData.slice(0, sortedData.length - products.length);
        setShowAlert(true);
        setErrorMessage(`Ada ${newTransactions.length} transaksi baru!`);
